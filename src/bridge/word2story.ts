...
async function WordToStory(wordList:string[]): Promise<StoryStruct> {
    try {
          ...
          messages: [
            {
              role: 'user',
              content: `
                Given a list of up to 10 words, ${JSON.stringify(wordList)}, generate a short English story(100words) that incorporates all the words. The output should be in JSON format with two key-value pairs: "title", "story", "titleZH" and "storyZH". The title should be a brief, engaging title for the story, and the story should be a coherent, creative narrative that uses all the provided words.
